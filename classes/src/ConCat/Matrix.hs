...
class MatrixMap f2 f1 where
  type Matrix f2 f1 :: Type -> Type
  linearMat :: (Additive s, Num s) => f1 s -> Matrix f2 f1 s -> f2 s
  linearVec :: (Additive s, Num s) => Matrix f2 f1 s -> f1 s -> f2 s
  outerVec :: Num s => f1 s -> f2 s -> Matrix f2 f1 s
  linearBoth :: (Additive s, Num s) => (f1 s, Matrix f2 f1 s) -> f2 s

instance KnownNat n => MatrixMap (Vector.Vector m) (Vector.Vector n) where
  type Matrix (Vector.Vector m) (Vector.Vector n) = Vector.Vector m :.: Vector.Vector n
